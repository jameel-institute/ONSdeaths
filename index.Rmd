...
producePlot<-function(name){
  ...
  p.name<-ggplot(name.data, aes(x=Week.ended, y=value,colour=variable))+ 
    ...
    geom_line()+
    scale_x_date(breaks=unique(name.data$Week.ended)[c(TRUE,FALSE,FALSE)], date_labels="%d/%m")+
...
if (floor(log10(current_week)) + 1 == 1){
  current_week = paste("0", current_week, sep = "")
}
...
#ONS_url = sprintf("https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/weeklyprovisionalfiguresondeathsregisteredinenglandandwales/2020/publishedweek4320202.xlsx")
# ONS_url2020 = sprintf("https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/weeklyprovisionalfiguresondeathsregisteredinenglandandwales/2020/publishedweek532020.xlsx")
# ONS_url = sprintf("https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/birthsdeathsandmarriages/deaths/datasets/weeklyprovisionalfiguresondeathsregisteredinenglandandwales/2020/publishedweek012021.xlsx")

# download.file(ONS_url2020, destfile=paste0("./data_dashboard/", "2020_results.xlsx"), quiet=T)
# data2020 <- read_xlsx("./data_dashboard/2020_results.xlsx", sheet='Weekly figures 2020', col_names = FALSE)
# data2020<-data2020[complete.cases(data2020[,3:8]),]
# data2020[is.na(data2020[,1]),1]<-data2020[is.na(data2020[,1]),2]
# k<-(1:nrow(data2020))[data2020$...1=="E12000001"][!is.na((1:nrow(data2020))[data2020$...1=="E12000001"])]
# data2020[(k:(k+9)),1]<-data2020[(k:(k+9)),2]
# data2020[,2]<-NULL
## data2020[data2020$...1=="week over the previous 5 years 1, 10, 11 (England and Wales)",1] <- 'Total deaths: average of corresponding week in 2015-2019 (England and Wales) 1'
## data2020[data2020$...1=="week over the previous 5 years 1, 10, 11 (England)",1] <- 'Total deaths: average of corresponding week in 2015-2019 (England) 1'
## data2020[data2020$...1=="week over the previous 5 years 1, 10, 11 (Wales)",1] <- 'Total deaths: average of corresponding week in 2015-2019 (Wales) 1'
# data2020[data2020$...1 == "Total deaths, all ages",1] <- "Total deaths, all ages (2020)"
#year <- c('year', rep(2020, ncol(data2020)-1))
#data2020 <- rbind(data2020, year)
# lastcolname <- 55

download.file(ONS_url, destfile=paste0("./data_dashboard/", "newest_results.xlsx"), quiet=T)
data<-read_xlsx("./data_dashboard/newest_results.xlsx", sheet='Weekly figures 2021', col_names = FALSE)
data <- data[complete.cases(data[,3]),] # 
data[is.na(data[,1]),1]<-data[is.na(data[,1]),2]
k<-(1:nrow(data))[data$...1=="E12000001"][!is.na((1:nrow(data))[data$...1=="E12000001"])]
data[(k:(k+9)),1]<-data[(k:(k+9)),2]
#data[,c(2,4:ncol(data))]<-NULL
data[,2]<-NULL
data[data$...1 == "Total deaths, all ages (2021)",1] <- "Total deaths, all ages"
data <- data[!(data$...1 == "Total deaths, all ages (2020)"),]
#year2021 <- c('year', 2021)
#data <- rbind(data, year2021)
# newcolnames <- paste('...', seq(lastcolname+1, lastcolname+(ncol(data)-1)), sep = "")
# colnames(data) <- c('...1', newcolnames)

# data <- full_join(data2020, data, by = '...1')

r<-c("<1", "1-4","5-9","10-14","15-19","20-24","25-29","30-34","35-39","40-44","45-49","50-54","55-59",                 "60-64","65-69","70-74", "75-79", "80-84", "85-89","90+")
...
write.table(data, file=paste0("./DataCleaned/mortality2021.csv"), row.names=FALSE, col.names=FALSE, sep=",")
data<-read.csv("./DataCleaned/mortality2021.csv",stringsAsFactors = FALSE)
...
df <- data %>% mutate(
  ...
  Average.5.years.England.and.Wales=Total.deaths..average.of.corresponding.week.in.2015.2019..England.and.Wales..1,
    #week.over.the.previous.5.years.1..10..11..England.and.Wales.,
  Average.5.years.England=Total.deaths..average.of.corresponding.week.in.2015.2019..England..1,
  #week.over.the.previous.5.years.1..10..11..England.,
  Average.5.years.Wales= Total.deaths..average.of.corresponding.week.in.2015.2019..Wales..1,
...
write.csv(df, file="./DataCleaned/mortality2021.csv", row.names=FALSE)
...
# data2020 <- read_xlsx("./data_dashboard/2020_results.xlsx", sheet='Covid-19 - Weekly registrations', col_names = FALSE,range='A5:BC86')
# data2020<-data2020[complete.cases(data2020[,3:8]),]
# data2020[is.na(data2020[,1]),1]<-data2020[is.na(data2020[,1]),2]
# k<-(1:nrow(data2020))[data2020$...1=="E12000001"]
# data2020[(k:(k+9)),1]<-data2020[(k:(k+9)),2]
# data2020[,2]<-NULL
# #data2020 <- rbind(data2020, year)
# lastcolname <- 55
...
string_condition_recent <- function(row, other_deaths, time){
  if ((other_deaths - as.numeric(row[,5]) > 0) & (other_deaths - as.numeric(row[,7]) > 0) & !(other_deaths - as.numeric(row[,6]) < 0)){ # if non-covid deaths greater than expected and its bounds
    string = sprintf('We would have expected %s (95%% PI %s – %s) all-cause deaths for that %s. This implies that total excess deaths were between %s – %s, and comprised of %s (95%% CI %s – %s) non-COVID-19 excess deaths.', 
  ...
  else if ((other_deaths - as.numeric(row[,5]) > 0) & ( other_deaths - as.numeric(row[,7]) >= 0) & (other_deaths - as.numeric(row[,6]) <= 0)){ # if non-covid deaths greater than expected but within bounds
    string = sprintf('We would have expected %s (95%% PI %s – %s) all-cause deaths for that %s. This implies that total excess deaths were between %s – %s, and excess non-COVID-19 deaths were not significantly different than the expected deaths.',
                     as.character(round(row[,5])),
                     as.character(round(row[,7])),  
                     as.character(round(row[,6])),  
                     time,
  ...
  else if ((other_deaths - as.numeric(row[,5]) < 0) & (other_deaths - as.numeric(row[,7]) > 0) & !(other_deaths - as.numeric(row[,6]) < 0)){ # if non-covid deaths less than expected 
    string = sprintf('There was a deficit of excess non-COVID-19 deaths in this %s of %s (95%% PI %s – %s), which means that non-COVID-19 deaths were significantly lower than the %s (95%% PI %s – %s) expected deaths for this %s.',
                    time,
                    as.character(round(other_deaths - as.numeric(row[,5]))),
                    as.character(round(other_deaths - as.numeric(row[,6]))),
                    as.character(round(other_deaths - as.numeric(row[,7]))),
...
```{r, echo=F, warning=F, message=FALSE, fig.height=12, fig.width = 12}
...
Regional_temp<-Regional_temp[Regional_temp$Week.ended==max(Regional_temp$Week.ended),]
...
colnames(regional_tab) <- c("Geographical unit", "Reported all-cause deaths", "Expected (%)", "Expected 95% prediction interval (%)", "COVID-19 (%)",
                            "Excess non-COVID-19 (%)", "Excess non-COVID-19 95% prediction interval (%)")
...
Agegrp_temp <-Agegrp_temp[Agegrp_temp$Week.ended==max(Agegrp_temp$Week.ended),]
...
colnames(agegrp_tab) <- c("Sex and age group (years)", "Reported all-cause deaths", "Expected (%)", "Expected 95% prediction interval (%)", "COVID-19 (%)",
                            "Excess non-COVID-19 (%)", "Excess non-COVID-19 95% prediction interval (%)")
