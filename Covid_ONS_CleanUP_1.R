...
###################
data<-read_xlsx(file2020,sheet='Covid-19 - Weekly registrations' ,col_names = FALSE,range='A5:BC86')
###################
...
write.table(data, file=paste0("./ONSCovid/Covid_Registration.csv"), row.names=FALSE, col.names=FALSE, sep=",")

# Clean up the COVID-19 Registrations data further
data<-read.csv("./ONSCovid/Covid_Registration.csv",stringsAsFactors = FALSE)
...
write.table(df, file=paste0("./ONSCovid/Covid_Registration.csv"), row.names=FALSE, sep=",")


### Read and clean the COVID-19 Occurrences sheet
##################
data<-read_xlsx(file2020,sheet='Covid-19 - Weekly occurrences' ,col_names = FALSE,range='A5:BC86')
###################
data<-data[complete.cases(data[,3:8]),]
data[is.na(data[,1]),1]<-data[is.na(data[,1]),2]
k<-(1:nrow(data))[data$...1=="E12000001"]
...
write.table(data, file=paste0("./ONSCovid/Covid_Occurrences.csv"), row.names=FALSE, col.names=FALSE, sep=",")


# Clean the COVID-19 Occurrences data further

data<-read.csv("./ONSCovid/Covid_Occurrences.csv",stringsAsFactors = FALSE)
...
write.table(df, file=paste0("./ONSCovid/Covid_Occurrences.csv"), row.names=FALSE, sep=",")
